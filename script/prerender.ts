...
export async function gerar() {
  ...
  const { renderizarRota, caminhos, origem, llms } = (await import(
    ...
    renderizarRota: (p: string) => { corpo: string; cabeca: string; htmlLang: string };
    caminhos: string[];
    origem: string;
    llms: string;
  };
  ...
  for (const rota of caminhos) {
    const { corpo, cabeca, htmlLang } = renderizarRota(rota);
    let html = molde;
    ...
    // Cada rota tem seu idioma: /en precisa nascer com lang="en", senão o
    // Google le a pagina inteira como portugues.
